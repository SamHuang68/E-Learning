-- Existing-project migration: add CS + Chinese progress to user_progress.
...
alter table public.user_progress
  add column if not exists cs
    jsonb not null default '{}'::jsonb,
  add column if not exists chinese
    jsonb not null default '{}'::jsonb;
...
begin
  ...
  where a.attrelid = 'public.user_progress'::regclass
    ...
    and a.attname in ('cs', 'chinese');

  if v_count <> 2 then
    raise exception 'CS/Chinese column count is %, expected 2', v_count;
  end if;

  if v_bad is not null then
    raise exception 'Incompatible CS/Chinese column contract: %', v_bad;
  end if;
end
$contract$;
