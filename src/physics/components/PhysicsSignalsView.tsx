...
function loadMasteryFromStorage(): MasteryMap {
  try {
    const raw = localStorage.getItem(STORAGE_KEY_PHYSICS_MASTERY)
    if (raw) {
      return JSON.parse(raw) as MasteryMap
    }
  } catch (err) {
    console.error('Failed to load physics signals mastery:', err)
  }
  return {}
}
...
function saveMasteryToStorage(map: MasteryMap): void {
  try {
    localStorage.setItem(STORAGE_KEY_PHYSICS_MASTERY, JSON.stringify(map))
    notifyProgressChanged()
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('physics:signals-mastery-updated'))
    }
  } catch (err) {
    console.error('Failed to save physics signals mastery:', err)
  }
}

/**
 * 物理 3 秒破題訊號決策卡與翻轉測驗元件 (PhysicsSignalsView)
 *
 * 功能亮點：
...
export const PhysicsSignalsView: React.FC = () => {
  ...
  return (
    <div
      className="physics-signals-container"
      style={{
        display: 'flex',
          ...
          >
            <button
              ...
              style={{
                ...
                color: viewMode === 'cards' ? '#0369a1' : '#e0f2fe',
            ...
            <button
              ...
              style={{
                ...
                color: viewMode === 'drill' ? '#0369a1' : '#e0f2fe',
      ...
      {viewMode === 'cards' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', minWidth: 0 }}>
          ...
          {filteredSignals.length === 0 ? (
            ...
            >
              {filteredSignals.map((sig: PhysicsSolvingSignal) => {
                const status = masteryMap[sig.id]
                return (
                  <div
                    key={sig.id}
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{
                  background: isShuffled ? '#e0f2fe' : '#f8fafc',
                  border: `1px solid ${isShuffled ? '#7dd3fc' : '#cbd5e1'}`,
                  color: isShuffled ? '#0369a1' : 'var(--ink)',
                  borderRadius: '6px',
                ...
                >
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', minWidth: 0 }}>
                      ...
                      <h3 style={{ margin: 0, fontSize: '0.95rem', color: '#0369a1' }}>
