...
import { LETTER_OF_CREDIT_SCENARIOS, type LetterOfCreditScenarioItem } from '../data/letterOfCreditDialogues'
...
export const LetterOfCreditLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: LetterOfCreditScenarioItem =
    LETTER_OF_CREDIT_SCENARIOS[selectedIdx % LETTER_OF_CREDIT_SCENARIOS.length]

  function speakLcAudio(text: string) {
  ...
  return (
    <div className="math-lab letter-of-credit-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>📜</span> {isJa ? 'TOEIC 信用状（L/C）・ディスクレ特訓' : 'TOEIC 國際信用狀 UCP 600 單證瑕疵聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '貿易金融＆決済・Part 3＆Part 7頻出！「信用状（L/C）・書類不一致（discrepancy）・荷渡保証書（shipping guarantee）」を完全制覇！'
              : '多益高階國際貿易考點：跟單信用狀嚴格相符原則、提單遲延裝運、開證行拒付與銀行擔保提貨！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {LETTER_OF_CREDIT_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.tradeFinanceDirector}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.documentaryCreditOfficer}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakLcAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.letterOfCreditKeywordsTipsJa}
          </div>
        </div>
