...
import { MNA_SCENARIOS, type MnaScenarioItem } from '../data/mnaDialogues'
...
export const MnaLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: MnaScenarioItem =
    MNA_SCENARIOS[selectedIdx % MNA_SCENARIOS.length]

  function speakMnaAudio(text: string) {
  ...
  return (
    <div className="math-lab mna-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🤝</span> {isJa ? 'TOEIC M&A（企業買収・合併）＆デューデリ特訓' : 'TOEIC 商務企業併購與盡職調查聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '最難関Part 7長文・Part 3対話頻出！「デューデリジェンス（due diligence）・秘密保持（NDA）・独禁法審査（antitrust）・シナジー（synergies）」を完全制覇！'
              : '多益高階商務重頭戲：企業併購盡職調查、反壟斷主管機關核准、保密協議與股東大會表決！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.managingPartner}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.leadFinancialAuditor}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakMnaAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.mnaKeywordsTipsJa}
          </div>
        </div>
