...
import { SUPPLY_CHAIN_SCENARIOS, type SupplyChainScenarioItem } from '../data/supplyChainDialogues'
...
export const SupplyChainLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: SupplyChainScenarioItem =
    SUPPLY_CHAIN_SCENARIOS[selectedIdx % SUPPLY_CHAIN_SCENARIOS.length]

  function speakSupplyChainAudio(text: string) {
  ...
  return (
    <div className="math-lab supply-chain-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🚢</span> {isJa ? 'TOEIC サプライチェーン・物流＆在庫管理特訓' : 'TOEIC 商務供應鏈物流與庫存管理聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'リスニングPart 3/4の頻出話題！「通関遅延（customs backlog）・安全在庫（safety stock）・緊急航空便（air freight）・支払い条件（Net 30）」を完全制覇！'
              : '多益高頻物流採購題型：海關延遲查驗、產線停擺危機、部分緊急空運與採購合約付款條款！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {SUPPLY_CHAIN_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.logisticsManager}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.procurementLead}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakSupplyChainAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.supplyChainKeywordsTipsJa}
          </div>
        </div>
