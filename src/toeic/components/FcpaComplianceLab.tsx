...
import { FCPA_COMPLIANCE_SCENARIOS, type FcpaComplianceScenarioItem } from '../data/fcpaComplianceDialogues'
...
export const FcpaComplianceLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: FcpaComplianceScenarioItem =
    FCPA_COMPLIANCE_SCENARIOS[selectedIdx % FCPA_COMPLIANCE_SCENARIOS.length]

  function speakFcpaAudio(text: string) {
  ...
  return (
    <div className="math-lab fcpa-compliance-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>⚖️</span> {isJa ? 'TOEIC FCPA・反贈賄コンプライアンス特訓' : 'TOEIC 反海外腐敗法 (FCPA) 企業合規聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '企業法務・Part 3＆Part 7頻出！「米FCPA（海外腐敗行為防止法）・贈賄レッドフラッグ・代理店デューデリジェンス」を完全制覇！'
              : '多益高階跨國法規考點：FCPA 反貪腐審查、第三方顧問紅旗警訊、禁止疏通費與企業誠信！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {FCPA_COMPLIANCE_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.chiefComplianceOfficer}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.regionalSalesDirector}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakFcpaAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.fcpaComplianceKeywordsTipsJa}
          </div>
        </div>
