...
import { TRADE_SCENARIOS, type TradeScenarioItem } from '../data/tradeDialogues'
...
export const TradeLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: TradeScenarioItem =
    TRADE_SCENARIOS[selectedIdx % TRADE_SCENARIOS.length]

  function speakTradeAudio(text: string) {
  ...
  return (
    <div className="math-lab trade-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🌐</span> {isJa ? 'TOEIC 国際貿易・インコタームズ＆通関特訓' : 'TOEIC 商務國際貿易與海關關稅聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'リスニング・読解の難関！「FOB vs CIF条件・船荷証券（B/L）・原産地証明書・関税（tariff）」を直感マスター！'
              : '多益高階國貿題型：國貿條規 FOB 與 CIF 責任分界、海上保險、提單與原產地證明！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.exportManager}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.importComplianceLead}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakTradeAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.tradeKeywordsTipsJa}
          </div>
        </div>
