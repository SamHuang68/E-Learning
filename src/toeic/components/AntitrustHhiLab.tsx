...
import { ANTITRUST_HHI_SCENARIOS, type AntitrustHhiScenarioItem } from '../data/antitrustHhiDialogues'
...
export const AntitrustHhiLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: AntitrustHhiScenarioItem =
    ANTITRUST_HHI_SCENARIOS[selectedIdx % ANTITRUST_HHI_SCENARIOS.length]

  function speakHhiAudio(text: string) {
  ...
  return (
    <div className="math-lab antitrust-hhi-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>📊</span> {isJa ? 'TOEIC 反トラスト・HHI市場集中度特訓' : 'TOEIC 反托拉斯 HHI 市場集中度指數聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '企業法務＆M&A・Part 3＆Part 7頻出！「HHI指数（市場集中度）・DOJ（司法省）差し止め訴訟・資産売却（divestiture）」を完全制覇！'
              : '多益高階企業併購考點：HHI 赫芬達爾指數精算、反競爭推定、司法部初步禁制令與資產剝離！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {ANTITRUST_HHI_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.seniorAntitrustEconomist}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakHhiAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.antitrustHhiKeywordsTipsJa}
          </div>
        </div>
