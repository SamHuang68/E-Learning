...
import { CONFLICT_MINERALS_SCENARIOS, type ConflictMineralsScenarioItem } from '../data/conflictMineralsDialogues'
...
export const ConflictMineralsLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: ConflictMineralsScenarioItem =
    CONFLICT_MINERALS_SCENARIOS[selectedIdx % CONFLICT_MINERALS_SCENARIOS.length]

  function speakConflictMineralsAudio(text: string) {
  ...
  return (
    <div className="math-lab conflict-minerals-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>💎</span> {isJa ? 'TOEIC 紛争鉱物＆労働監査特訓' : 'TOEIC 供應鏈無衝突礦產與勞工人權聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'ESGサプライチェーン・Part 3＆Part 7頻出！「紛争鉱物（conflict-free 3TG）・精錬所認証（smelters）・労働監査（ILO audit）・是正措置（remediation）」を完全制覇！'
              : '多益高階供應鏈與社會責任考點：無衝突礦產驗證、RMI 冶煉廠認證、ILO 勞工準則與突擊稽核！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {CONFLICT_MINERALS_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.procurementDirector}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.esgAuditLead}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #0ea5e9, #0284c7)',
              }}
              onClick={() => speakConflictMineralsAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.conflictMineralsKeywordsTipsJa}
          </div>
        </div>
