...
import { REAL_ESTATE_SCENARIOS, type RealEstateScenarioItem } from '../data/realEstateDialogues'
...
export const RealEstateLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: RealEstateScenarioItem =
    REAL_ESTATE_SCENARIOS[selectedIdx % REAL_ESTATE_SCENARIOS.length]

  function speakRealEstateAudio(text: string) {
  ...
  return (
    <div className="math-lab real-estate-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🏢</span> {isJa ? 'TOEIC 不動産・オフィス賃貸＆移転交渉特訓' : 'TOEIC 商務房地產租賃與辦公室擴遷聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'ビジネス読解・リスニング必須！「オフィス賃貸契約（commercial lease）・フリーレント（rent-free period）・坪単価（sq ft rate）・空調（HVAC）」を完全攻略！'
              : '多益高頻商辦題目：辦公室租賃合約、免租裝修期、每平方英尺租金、全天候空調與物業設施！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {REAL_ESTATE_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.propertyBroker}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.operationsDirector}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakRealEstateAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.realEstateKeywordsTipsJa}
          </div>
        </div>
