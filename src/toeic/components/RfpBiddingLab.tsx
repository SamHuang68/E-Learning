...
import { RFP_SCENARIOS, type RfpScenarioItem } from '../data/rfpBiddingDialogues'
...
export const RfpBiddingLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: RfpScenarioItem =
    RFP_SCENARIOS[selectedIdx % RFP_SCENARIOS.length]

  function speakRfpAudio(text: string) {
  ...
  return (
    <div className="math-lab rfp-bidding-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>📑</span> {isJa ? 'TOEIC 調達RFP＆入札ベンダー評価特訓' : 'TOEIC 商務採購 RFP 競標與供應商評選聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'ビジネス契約・Part 4＆Part 7頻出！「提案依頼書（RFP）・封印入札（sealed bids）・加権評価マトリクス・遅延違約金（liquidated damages）」を完全制覇！'
              : '多益高階商務採購考點：RFP 徵求建議書、密封投標比價、加權評分矩陣與延遲履約違約金！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            <span style={{ fontSize: '0.72rem', padding: '0.1rem 0.45rem', borderRadius: '999px', background: 'rgba(59, 130, 246, 0.15)', color: '#2563eb', fontWeight: 700 }}>
              {activeItem.accentLabel}
            </span>
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.chiefProcurementOfficer}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.seniorContractsNegotiator}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #2563eb, #1d4ed8)',
              }}
              onClick={() => speakRfpAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.rfpKeywordsTipsJa}
          </div>
        </div>
