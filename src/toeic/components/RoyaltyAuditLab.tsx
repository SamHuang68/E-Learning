...
import { ROYALTY_AUDIT_SCENARIOS, type RoyaltyAuditScenarioItem } from '../data/royaltyAuditDialogues'
...
export const RoyaltyAuditLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: RoyaltyAuditScenarioItem =
    ROYALTY_AUDIT_SCENARIOS[selectedIdx % ROYALTY_AUDIT_SCENARIOS.length]

  function speakRoyaltyAudio(text: string) {
  ...
  return (
    <div className="math-lab royalty-audit-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>📊</span> {isJa ? 'TOEIC ライセンス料監査特訓' : 'TOEIC 專利權利金審計與追補條款聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '知財ライセンス・Part 3＆Part 7頻出！「ロイヤルティ監査（royalty audit）・過少申告（underreported）・監査費用転嫁条項」を完全制覇！'
              : '多益高階智財授權法務考點：專利權利金審計、短報銷售額追討、延遲利息與審計成本移轉！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {ROYALTY_AUDIT_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(234, 88, 12, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '📜' : '📊'}
          </div>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.intellectualPropertyLicensingDirector}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.seniorAuditPartner}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #ea580c, #c2410c)',
              }}
              onClick={() => speakRoyaltyAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.royaltyAuditKeywordsTipsJa}
          </div>
        </div>
