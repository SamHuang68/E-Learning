...
import { CLOUD_SLA_SCENARIOS, type CloudSlaScenarioItem } from '../data/cloudSlaDialogues'
...
export const CloudSlaLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: CloudSlaScenarioItem =
    CLOUD_SLA_SCENARIOS[selectedIdx % CLOUD_SLA_SCENARIOS.length]

  function speakSlaAudio(text: string) {
  ...
  return (
    <div className="math-lab cloud-sla-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>☁️</span> {isJa ? 'TOEIC クラウドSLA＆障害補償特訓' : 'TOEIC 雲端服務協議 (SLA) 與停機補償聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'クラウドIT契約・Part 3＆Part 7頻出！「SLA 99.99%稼働率保証（uptime commitment）・サービスクレジット（service credits）・フェイルオーバー（failover）」を完全制覇！'
              : '多益高階雲端運算考點：SLA 99.99% 承諾、非計畫停機補償、服務點數折抵與容錯備援！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {CLOUD_SLA_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.chiefInformationOfficer}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.cloudAccountExecutive}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakSlaAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.cloudSlaKeywordsTipsJa}
          </div>
        </div>
