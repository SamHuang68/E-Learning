...
import { FORCE_MAJEURE_SCENARIOS, type ForceMajeureScenarioItem } from '../data/forceMajeureDialogues'
...
export const ForceMajeureLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: ForceMajeureScenarioItem =
    FORCE_MAJEURE_SCENARIOS[selectedIdx % FORCE_MAJEURE_SCENARIOS.length]

  function speakForceMajeureAudio(text: string) {
  ...
  return (
    <div className="math-lab force-majeure-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🌪️</span> {isJa ? 'TOEIC 不可抗力条項＆保険損害査定特訓' : 'TOEIC 商務不可抗力與保險理賠聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'ビジネス契約・Part 3＆Part 7頻出！「不可抗力（force majeure）・天変地異（act of God）・損害査定人（claims adjuster）・免責額（deductible）」を完全制覇！'
              : '多益高頻商務法規考點：合約不可抗力免責條款、天災履約寬限、保險自負額與公證理賠！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {FORCE_MAJEURE_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(14, 165, 233, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '📜' : '🌪️'}
          </div>

          <div>
            <span style={{ fontSize: '0.72rem', padding: '0.1rem 0.45rem', borderRadius: '999px', background: 'rgba(14, 165, 233, 0.15)', color: '#0284c7', fontWeight: 700 }}>
              {activeItem.accentLabel}
            </span>
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.corporateRiskOfficer}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.insuranceClaimsAdjuster}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #0284c7, #0369a1)',
              }}
              onClick={() => speakForceMajeureAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.forceMajeureKeywordsTipsJa}
          </div>
        </div>
