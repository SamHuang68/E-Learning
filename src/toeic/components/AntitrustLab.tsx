...
import { ANTITRUST_SCENARIOS, type AntitrustScenarioItem } from '../data/antitrustDialogues'
...
export const AntitrustLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: AntitrustScenarioItem =
    ANTITRUST_SCENARIOS[selectedIdx % ANTITRUST_SCENARIOS.length]

  function speakAntitrustAudio(text: string) {
  ...
  return (
    <div className="math-lab antitrust-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>⚖️</span> {isJa ? 'TOEIC 独占禁止法＆カルテル防止特訓' : 'TOEIC 商務反托拉斯壟斷與價格合謀防範聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? 'ビジネス法務・Part 3＆Part 7頻出！「独占禁止法（antitrust）・価格カルテル（price-fixing）・市場分割（market allocation）・内部通報（whistleblower）」を完全制覇！'
              : '多益高階商務法規與監管考點：反托拉斯合規、價格合謀防範、產能分配限制與吹哨者通報程序！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {ANTITRUST_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(234, 179, 8, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '📜' : '⚖️'}
          </div>
          ...
          <div>
            ...
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.seniorLegalCounsel}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #eab308, #ca8a04)',
              }}
              onClick={() => speakAntitrustAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.antitrustKeywordsTipsJa}
          </div>
        </div>
