...
import { BUSINESS_INTERRUPTION_SCENARIOS, type BusinessInterruptionScenarioItem } from '../data/businessInterruptionDialogues'
...
export const BusinessInterruptionLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: BusinessInterruptionScenarioItem =
    BUSINESS_INTERRUPTION_SCENARIOS[selectedIdx % BUSINESS_INTERRUPTION_SCENARIOS.length]

  function speakBiiAudio(text: string) {
  ...
  return (
    <div className="math-lab business-interruption-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🏭</span> {isJa ? 'TOEIC 休業損害保険（BII）理賠特訓' : 'TOEIC 商業營業中斷險 (BII) 理賠聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '企業リスク管理・Part 3＆Part 7頻出！「休業損害保険（BII）・72時間免責期間・固定費補償（continuing expenses）」を完全制覇！'
              : '多益高階企業風險管理考點：營業中斷毛利損失、72小時免賠等待期、法證理賠公估與臨時移地開銷！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {BUSINESS_INTERRUPTION_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(217, 119, 6, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '📜' : '🏭'}
          </div>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.corporateRiskManager}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.seniorForensicClaimsAdjuster}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #d97706, #b45309)',
              }}
              onClick={() => speakBiiAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.businessInterruptionKeywordsTipsJa}
          </div>
        </div>
