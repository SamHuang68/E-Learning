...
import { NDA_TRADE_SECRETS_SCENARIOS, type NdaTradeSecretsScenarioItem } from '../data/ndaTradeSecretsDialogues'
...
export const NdaTradeSecretsLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: NdaTradeSecretsScenarioItem =
    NDA_TRADE_SECRETS_SCENARIOS[selectedIdx % NDA_TRADE_SECRETS_SCENARIOS.length]

  function speakNdaAudio(text: string) {
  ...
  return (
    <div className="math-lab nda-trade-secrets-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🤝</span> {isJa ? 'TOEIC 秘密保持契約（NDA）特訓' : 'TOEIC 商務保密協議與營業秘密聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '企業法務・Part 3＆Part 7頻出！「秘密保持契約（NDA）・営業秘密（trade secret）・予定損害賠償額（liquidated damages）」を完全制覇！'
              : '多益高階商務法務考點：雙向保密協議、營業秘密保護、五年存續期與預定違約金條款！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {NDA_TRADE_SECRETS_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.corporateLegalCounsel}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.businessDevelopmentDirector}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakNdaAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.ndaTradeSecretsKeywordsTipsJa}
          </div>
        </div>
