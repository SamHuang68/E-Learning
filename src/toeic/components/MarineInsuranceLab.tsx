...
import { MARINE_INSURANCE_SCENARIOS, type MarineInsuranceScenarioItem } from '../data/marineInsuranceDialogues'
...
export const MarineInsuranceLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: MarineInsuranceScenarioItem =
    MARINE_INSURANCE_SCENARIOS[selectedIdx % MARINE_INSURANCE_SCENARIOS.length]

  function speakMarineAudio(text: string) {
  ...
  return (
    <div className="math-lab marine-insurance-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🚢</span> {isJa ? 'TOEIC 海上保険＆共同海損特訓' : 'TOEIC 海運貨物保險與共同海損聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '国際貿易・Part 3＆Part 7頻出！「共同海損（General Average）・貨物投棄（jettison）・保証状（GA Guarantee）・ヨーク・アントワープ規則」を完全制覇！'
              : '多益高階國際貿易法務考點：共同海損宣告、貨物應急拋海、共同海損保證函與分攤理算！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {MARINE_INSURANCE_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.maritimeClaimsDirector}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.freightLogisticsManager}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakMarineAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.marineInsuranceKeywordsTipsJa}
          </div>
        </div>
