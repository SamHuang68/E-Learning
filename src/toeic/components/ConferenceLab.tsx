...
export const ConferenceLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  return (
    <div className="math-lab conference-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '14px', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(99, 102, 241, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '🎙️' : '💻'}
          </div>

          <div>
            <span style={{ fontSize: '0.72rem', padding: '0.1rem 0.45rem', borderRadius: '999px', background: 'rgba(99, 102, 241, 0.15)', color: '#818cf8', fontWeight: 700 }}>
              {activeItem.accentLabel}
            </span>
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.speaker}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.host}</span>
          </div>

          {/* 會議控制模擬列 */}
          <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', background: 'var(--surface-soft)', padding: '0.35rem 0.6rem', borderRadius: '999px', border: '1px solid var(--line)' }}>
            <button
              type="button"
              className="pill-btn"
            ...
            <span style={{ fontSize: '0.72rem', color: isMicMuted ? '#ef4444' : '#10b981', fontWeight: 600 }}>
              {isMicMuted ? 'You are on mute' : 'Mic Active'}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #6366f1, #818cf8)',
              }}
