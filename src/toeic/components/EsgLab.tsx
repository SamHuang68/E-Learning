...
import { ESG_SCENARIOS, type EsgScenarioItem } from '../data/esgDialogues'
...
export const EsgLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: EsgScenarioItem =
    ESG_SCENARIOS[selectedIdx % ESG_SCENARIOS.length]

  function speakEsgAudio(text: string) {
  ...
  return (
    <div className="math-lab esg-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>🌱</span> {isJa ? 'TOEIC ESG・脱炭素＆カーボンオフセット特訓' : 'TOEIC 商務企業永續 ESG 與碳盤查聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '最先端Part 4・Part 7必須！「Scope 1/2/3排出量算定・炭素国境調整（CBAM）・カーボンクレジット（offset credits）」を徹底マスター！'
              : '多益高頻綠色商務考點：範疇一二三溫室氣體排放盤查、歐盟碳邊境稅、第三方獨立永續報告查證！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: isPlaying ? 'rgba(16, 185, 129, 0.2)' : 'rgba(5, 150, 105, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
            {isPlaying ? '🌍' : '🌱'}
          </div>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.chiefSustainabilityOfficer}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.supplyChainDecarbonizationLead}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              style={{
                ...
                borderRadius: '999px',
                background: isPlaying ? '#10b981' : 'linear-gradient(135deg, #059669, #047857)',
              }}
              onClick={() => speakEsgAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.esgKeywordsTipsJa}
          </div>
        </div>
