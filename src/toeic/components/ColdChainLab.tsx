...
import { COLD_CHAIN_SCENARIOS, type ColdChainScenarioItem } from '../data/coldChainDialogues'
...
export const ColdChainLab: React.FC<Props> = ({ onEarnXp, instructionLang = 'zh' }) => {
  ...
  const activeItem: ColdChainScenarioItem =
    COLD_CHAIN_SCENARIOS[selectedIdx % COLD_CHAIN_SCENARIOS.length]

  function speakColdChainAudio(text: string) {
  ...
  return (
    <div className="math-lab cold-chain-lab" style={{ width: '100%', maxWidth: '100%', minWidth: 0 }}>
      {/* 標頭 */}
      <div className="lab-header" style={{ marginBottom: '0.8rem' }}>
        <div>
          <h3 style={{ margin: '0 0 0.2rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span>❄️</span> {isJa ? 'TOEIC 航空貨物＆コールドチェーン物流特訓' : 'TOEIC 商務國際空運與冷鏈物流聽力實驗室'}
          </h3>
          <p className="lab-desc" style={{ margin: 0, fontSize: '0.78rem', color: 'var(--muted)', lineHeight: 1.4 }}>
            {isJa
              ? '国際物流・Part 4＆Part 7頻出！「航空運送状（AWB）・超低温冷凍輸送（-70℃）・温度逸脱（temperature excursion）・貨物保険請求」を完全制覇！'
              : '多益高難度商務空運考點：主空運提單追蹤、生技疫苗超低溫冷鏈、溫度失控隔離通報與貨損理賠！'}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        {COLD_CHAIN_SCENARIOS.map((item, idx) => (
          <button
            key={item.id}
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.8rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '1.1rem', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div>
            ...
            <h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.05rem' }}>{activeItem.dialogueRoles.freightOperationsDirector}</h3>
            <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>{activeItem.dialogueRoles.coldChainLogisticsManager}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <button
              ...
              onClick={() => speakColdChainAudio(activeItem.audioScript)}
            >
          ...
          <div style={{ marginTop: 'auto', background: 'var(--surface-soft)', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--line)', fontSize: '0.74rem', color: 'var(--muted)', textAlign: 'left', lineHeight: 1.45 }}>
            💡 <strong>TOEIC 頻出ポイント：</strong>{activeItem.coldChainKeywordsTipsJa}
          </div>
        </div>
