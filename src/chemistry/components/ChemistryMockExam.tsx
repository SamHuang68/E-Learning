...
/**
 * 判斷學生化學作答是否正確 (支援單選 choice、多選 multi-choice 與填充 fill)
 */
...
/**
 * 臺灣 108 課綱化學全真模擬考試系統 (ChemistryMockExam)
 */
export const ChemistryMockExam: React.FC<Props> = ({
  onSaveScore,
  onNavigateVault,
}) => {
  ...
  const exam: ChemistryMockExamType =
    exams.find((e) => e.id === selectedExamId) || exams[0]
  const initialSeconds = (exam.durationMinutes || 40) * 60
  ...
  useEffect(() => {
    ...
    setTimeLeft((exam.durationMinutes || 40) * 60)
  }, [selectedExamId, exam.durationMinutes])
  ...
  const handleCalculateScore = useCallback(
    (forceSubmit = false) => {
      ...
      try {
        const currentProg: ChemistryProgressState = loadChemistryProgress()
        ...
        const updatedProgress: ChemistryProgressState = {
          ...currentProg,
        ...
        saveChemistryProgress(updatedProgress)
  ...
  const diagnosticReport = useMemo(() => {
    ...
    const strandMap: Record<
      string,
      { total: number; correct: number; strand: ChemistryStrand }
    > = {}
    ...
    exam.questions.forEach((q) => {
      ...
      const strand = q.strand || 'matter_structure'
      if (!strandMap[strand]) {
        ...
        {exams.map((ex) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              background:
                selectedExamId === ex.id ? '#059669' : 'var(--surface-subtle)',
      ...
      >
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            gap: '0.4rem',
            background: isTimeUrgent ? '#fef2f2' : '#ecfdf5',
            border: `1px solid ${isTimeUrgent ? '#f87171' : '#a7f3d0'}`,
            borderRadius: '999px',
        ...
        >
          <span
            style={{
              fontSize: '0.85rem',
              fontWeight: 700,
              color: isTimeUrgent ? '#dc2626' : '#047857',
            }}
          >
                    ...
                    if (isSubmitted) {
                      ...
                      const isOptionTarget =
                        typeof q.answer === 'number'
                          ? q.answer === oIdx
                          : String(q.answer).trim().toUpperCase() === letter
