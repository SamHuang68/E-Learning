...
describe('i18n dictionary', () => {
  ...
  it('translates primary chrome holes Codex flagged', () => {
    ...
    expect(translate('zh-Hant', 'hub.bottomNav.home')).toBe('回到大廳')
    expect(translate('en', 'hub.bottomNav.home')).toBe('Back to hall')
    expect(translate('en', 'hub.bottomNav')).toMatch(/bottom/i)
    expect(translate('zh-Hant', 'hub.bottomNav.tracks')).toBe('全部軌道')
    expect(translate('zh-Hant', 'hub.bottomNav.about')).toBe('隱私與資料說明')
    expect(translate('en', 'hub.bottomNav.about')).toMatch(/privacy/i)
    expect(translate('zh-Hant', 'hub.github')).toMatch(/E-Learning/)
    expect(translate('en', 'hub.github')).toMatch(/E-Learning/)
    expect(translate('zh-Hant', 'hub.github')).not.toBe('GitHub')
    expect(translate('en', 'hub.github')).not.toBe('GitHub')
  })
  ...
  it('keeps Hub catalog track titles on the same glossary as chrome', () => {
    ...
    expect(EN['title.hub']).toBe('Practice hall')
    expect(ZH_HANT['title.hub']).toBe('練習大廳')
    expect(ZH_HANT['title.ja']).toBe('日語 · 練習')
    expect(EN['title.ja']).toBe('Japanese · practice')
    expect(ZH_HANT['title.calculus']).not.toMatch(/專題|Hub/)
    expect(EN['title.calculus']).not.toMatch(/studio|Hub/i)
