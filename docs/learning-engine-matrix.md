...
| 微積分 | `calculus` | **FSRS 4.5**（`LearningCoordinator` 作答後寫入） | **2PL IRT θ** 本機估計（非正式鑑定） | 幾何實驗室、推導步驟、4 階練習題 | 寫入數學進度的 `calculusTheta` / `calculusFsrs` / `calculusResponses` |
...
| 2PL IRT（`src/engine/adaptive.ts`） | 微積分 4 階練習的本機 θ 估計與選題輔助。UI 必須標「本機估計／非正式鑑定」。 |
